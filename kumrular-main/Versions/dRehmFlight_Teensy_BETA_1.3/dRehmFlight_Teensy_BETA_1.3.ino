...
float GyroErrorY= -1.91;
float GyroErrorZ = 1.79;
...
void controlMixer() {
  ...
  s3_command_scaled = thro_des+ (pitch_PID + pitch_passthru) * cos(120 * DEG2RAD) + (roll_PID + roll_passthru) * sin(120 * DEG2RAD) - (yaw_PID + yaw_passthru);
  s2_command_scaled = thro_des+ (pitch_PID + pitch_passthru) * cos(-120 * DEG2RAD) + (roll_PID + roll_passthru) * sin(-120 * DEG2RAD) + (yaw_PID + yaw_passthru);
  s1_command_scaled = thro_des + (pitch_PID + pitch_passthru) * cos(0) + (roll_PID + roll_passthru) * sin(0);
  s4_command_scaled = yaw_PID + yaw_passthru + 0.5;

  // ALPEREN
  // s3_command_scaled = thro_des+ (pitch_PID + pitch_passthru) * sin(30) + (roll_PID + roll_passthru) * sin(60) - (yaw_PID + yaw_passthru);
  // s2_command_scaled = thro_des+ (pitch_PID + pitch_passthru) * sin(30) + (roll_PID + roll_passthru) * -sin(60) + (yaw_PID + yaw_passthru);
  // s1_command_scaled = thro_des + (pitch_PID + pitch_passthru);
  // s4_command_scaled = yaw_PID + yaw_passthru + 0.5;

  //  MANUAL
  //  s1_command_scaled = thro_des + pitch_passthru * 0.667 + roll_passthru * 1 ;
  //  s2_command_scaled = thro_des + pitch_passthru * 0.667 - roll_passthru * 1 ;
  //  s3_command_scaled = thro_des + pitch_passthru * 1.334;
  //  s4_command_scaled = 0.5 + yaw_passthru ;
  
  //  PID ONLY
  //  s1_command_scaled = thro_des + pitch_PID * 0.667 + roll_PID * 1 ;
  //  s2_command_scaled = thro_des + pitch_PID * 0.667 - roll_PID * 1 ;
  //  s3_command_scaled = thro_des + pitch_PID * 1.334;
